...
display_sender::display_sender(int id, const std::string& ip_address, int port)
{
...
void display_sender::send(const cv::Mat& m)
{
	if(tcp_sender_->get_num_queued() > 1) return;
	tcp_sender_->send_data(serialize(m));
}

std::shared_ptr<std::vector<char>> display_sender::serialize(const cv::Mat& m)
{
